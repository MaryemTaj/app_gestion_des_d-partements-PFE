	...
	<div class="wrapper">
        ...
        <div class="card">
            <div class="card-bdoy">
				...
				$sth = $db->prepare("SELECT * from filiere f inner join cycle c on f.id_cycle=c.id_cycle inner join departement d on f.id_departement=d.id_dprtm inner join professeur p on f.coorFiliere=p.id_prof where f.isBrouillon=1");
				$sth->execute(); 
				$result = $sth->fetchAll();
      ...
      <tr class="table-primary ">
		...
		<?php foreach ($result as $result) {?>
			...
			<td><?php echo $result['listeDesModules']; ?></td>
			<td><?php echo $result['nomPrenom_prof']; ?></td>
			<td><?php echo $result['nom_cycle']; ?></td>
			<td><?php echo $result['nom_dprtm']; ?></td>
			<td>
            <a href="../Admin/genratepdf.php?Id=<?php echo $result['id_filiere'] ?>" target="_blank">
			        <i style="margin-left: 40%; font-size:18px; color:black;" class="fas fa-download"></i> 
			    </a>	</td>
		
			<td>
            <a href="BrouillonsVersModule.php?id=<?=$result['id_filiere']?>" type="button" class="btn btn-success">Ajouter module</a>
			</td>
