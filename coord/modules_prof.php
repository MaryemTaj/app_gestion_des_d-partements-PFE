...
<!---------------------------------------------- navbar & sidebar menu ------------------------------------------------------------------------->
    ...
    <div class="container-fluid pt-3">
        <div class="card">
			...
			$request = "SELECT  l.prof_cours  , l.prof_TD , l.prof_TP , f.intitule_filiere , m.intitule_module ,
			           l.semestre FROM liste_modules l INNER JOIN filiere f ON l.id_filiere = f.id_filiere INNER JOIN module m ON l.id_module=m.id_module  where l.prof_cours= '$coord' or l.prof_TD= '$coord' or l.prof_TP= '$coord' " ;
    ...
    <tbody>
		<?php foreach ($result as $result) {?>
			...
			<td><?php if($result['prof_cours'] == $coord){
				   echo'Cours';
			     }elseif($result['prof_TD'] == $coord){
			     	echo'TD';
			     }elseif($result['prof_TP'] == $coord){
			     	echo'TP';
			     } ?></td>
