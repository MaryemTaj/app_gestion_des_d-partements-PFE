...
if (isset($_SESSION['coordFiliere'])) {
	$sth = $db->prepare("SELECT * FROM `professeur` p where p.id_prof=:id_prof and p.id_prof in (select f.coorFiliere from filiere f inner join departement d on d.id_dprtm=f.id_departement where d.id_dprtm=:id_dprtm)");
		$sth->bindValue(':id_prof', $_SESSION['coordFiliere']['id_prof']);
        $sth->bindValue(':id_dprtm', $_POST['departement']);
        $sth->execute(); 
		$result = $sth->fetch();
		$count=$sth->rowCount();
	if ($_SESSION['coordFiliere']['anciennete']>6 && $_SESSION['coordFiliere']['grade']='A' && !empty($result)) {
		if(isset($_SESSION['nouvelleFiliere']['etape3'])){
			unset($_SESSION['nouvelleFiliere']['etape3']);
			}
			if(isset($_POST) && empty($_SESSION['nouvelleFiliere']['etape2'])){
				$_SESSION['nouvelleFiliere']['etape2']=$_POST;
				unset($_POST);
			}
							...
							<ul class="dropdown-menu dropdown-user animated fadeIn">
								<div class="dropdown-user-scroll scrollbar-outer">
									...
									<li>
										...
										<div class="dropdown-divider"></div>
										<a class="dropdown-item" href="../login/logoutCoord.php">Logout</a>
									</li>
								</div>
							</ul>
						</li>
		...
		<div class="main-panel">
			<div class="content">
                ...
                <div class="card">
                    <div class="card-body">
                        <?php
                        $sth = $db->prepare("SELECT nom_cycle,id_cycle from cycle ");
                        $sth->execute(); 
                        $row=$sth->fetchAll();
                        ...
                        ?>
    <form method="post" action="demandeFiliere3.php">
...
<?php
foreach ($row as $row) {?>
<option value="<?=$row['id_cycle']?>"><?=$row['nom_cycle']?></option>
<?php } ?>
    ...
    <div class="form-group col-md-4">
      ...
      <input type="text" required name="Etablissement" value="fsjes ain sebaa" class="form-control" id="Etablissement" placeholder="Etablissement">
    </div>
	<div class="form-group col-md-4">
      <label for="Departement">Departement d'attache de la filiere</label>
      <input type="text" required name="Departement" class="form-control" value="<?=$_SESSION['nouvelleFiliere']['etape2']['departement']?>" id="Departement" placeholder="Departement">
    </div>
  </div>
  <div class="form-group">
    <label for="intitule">intitule</label>
    <input type="text" required name="intitule" class="form-control" name="intitule" id="intitule" placeholder="intitule">
	<?php
  ...
  <div class="form-group">
    <label for="diplome">Specialite du diplome</label>
    <input type="text" required class="form-control" name="diplome" id="diplome" placeholder="diplome">
  </div>
  <div class="form-row">
    <div class="form-group">
	<label for="CoordDepartement">Coordonnateur de deprtement d'attache</label>
	<?php
	 $sth = $db->prepare("SELECT * FROM professeur p inner join departement d on p.id_prof=d.chefDepartement where d.id_dprtm=:dprtm;");
	  $sth->bindValue(':dprtm', $_SESSION['nouvelleFiliere']['etape2']['departement']); 
	  $sth->execute(); 
	  $result=$sth->fetch(PDO::FETCH_ASSOC);
	 
	?>
	<?= print_r($result);?>
      <input type="text" required value="<?=$result['nomPrenom_prof']??'Sans chef'?>" name="coordDepartement" class="form-control" >
    </div>
  </div>
  <span>
...
<script>
		...
		header("location:demandeFiliere.php?message=Vous n'avez pas le droit");
