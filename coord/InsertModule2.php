...
if (isset($_POST['newModule'])) {
  ...
  //  print_r($_SESSION['nouvelleFiliere']['etape5']);
      ...
      if(empty($result)){
          switch ($_POST['semestre']) {
              case 'Semestre 1' || 'Semestre 2' || 'Semestre 3':
                 
                  $stmt=$db->prepare("SELECT count(m.id_module)
            ...
            $stmt2->execute();
                       header('Location: InsertModuleMaster.php?message="Insertion Reussite"');
                       
                         
                        print_r($_POST);
                    }
                    else{
                        header('Location: InsertModuleMaster.php?message="Vous ave depassez le nombre de modules necessaire pour cette semestre"');
                    }
                  break;
        ...
        unset($_POST);
        header('Location: InsertModuleMaster.php?message="Module existe deja"');
      }
...
if(empty($result)){
    switch ($_POST['semestreExiste']) {
        case 'Semestre 1' || 'Semestre 2' || 'Semestre 3':
            ...
            inner JOIN filiere f on f.id_filiere=lm.id_filiere 
            where f.id_filiere=:id_filiere and lm.semestre=:semestre; ");
            $stmt->bindValue(':semestre', $_POST['semestreExiste']);
            $stmt->bindValue(':id_filiere', $_SESSION['nouvelleFiliere']['etape5']['id_filiere']);
            $stmt->execute(); 
          ...
          if($row['count(m.id_module)']<6){
            ...
            header('Location: InsertModuleMaster.php?message="Vous avez depassez le nombre de modules necessaire pour cette semestre"');
