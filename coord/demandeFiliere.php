...
if (isset($_SESSION['coordFiliere']) ) {
    if($_SESSION['coordFiliere']['anciennete']>6){
		if (isset($_SESSION['nouvelleFiliere'])) {
			unset($_SESSION['nouvelleFiliere']);
		}
...
<!---------------------------------------------- navbar & sidebar menu ------------------------------------------------------------------------->
  ...
  <div class="form-row">
    <div class="form-group col-md-6">
      ...
      <select class="form-control" name="departement"  id="dprtm">
<?php
foreach ($row as $row) {?>
<option value="<?=$row['id_dprtm']?>"><?=$row['nom_dprtm']?></option>
<?php } ?>
  ...
  <button type="submit" name="dprtm" class="btn btn-primary btn-block w-25 p-2 bd-highlight">suivant</button>
</div>
...
<?php
        ...
        $message="Vou n'avez pas le droit";
